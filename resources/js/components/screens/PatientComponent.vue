<template>
    <div class="col-md-12 ml-sm-auto col-lg-10 px-md-4">
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="staticBackdropLabel">{{ this.update ? 'Atualizando' : 'Inserindo' }} paciente</h5>
            ...
            <div class="modal fade" id="deleteModal" tabindex="-1" aria-labelledby="deleteModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <p>Você realmente deseja excluir este paciente do sistema?</p>
                        </div>
                        <div class="modal-footer">
        ...
        <table id="table_id" class="display">
            ...
            <tbody>
            <tr v-for="patient in this.patients.patients" :key="patient.id">
                <td>{{ patient.id }}</td>
                <td>{{ patient.name }}</td>
                <td>{{ patient.email }}</td>
                <td>{{ patient.phone }}</td>
                <td>
                    <i class="bi bi-pencil-square mr-3 action" @click="edit(patient.id)"></i>
                    <i class="bi bi-trash action" @click="openModalDelete(patient.id)"></i>
                </td>
            </tr>
            </tbody>
        </table>
        ...
        methods: {
            ...
            submit() {
                ...
                if (this.$v.$invalid) {
                    ...
                    let action = this.update ? 'updatePatient' : 'storePatient';
                ...
                this.$store.dispatch('getPatient', id)
                    .then(res => {
                        ...
                        if(res.data.profile_pic && res.data.profile_pic !== 'null') {
                            let path = [`/storage/patients/${res.data.profile_pic}`];
                            let blob = null;
